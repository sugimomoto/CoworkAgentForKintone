...
// Settings View の「同期」ボタンが Worker /skills/sync 経由で Anthropic に upload する。
// バージョン管理は Anthropic Workspace に集約 (display_title 一致なら新 version、
// 無ければ新規作成という Worker 側の挙動が source of truth)。
...
`;
  writeFileSync(outPath, ts);
  console.log(`[build] src/generated/skills-bundle.ts generated (${skills.length} skills)`);
}

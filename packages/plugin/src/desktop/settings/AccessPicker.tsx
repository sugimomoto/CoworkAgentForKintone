...
const AXES: AxisDef[] = [
  { key: 'allowedUsers',         kind: 'user',  label: 'ユーザー', unit: '人',       placeholder: '名前 / ログイン名で検索', tint: 'var(--cw-accent, #0d9488)' },
...
function useAxisSearch(search: AccessSearchFn, exclude: string[]) {
  ...
  // unmount 時に debounce タイマーを片付ける (= 閉じた modal 内で setResults を呼ばない)
...
function AxisSearchField({
  ...
  useEffect(() => {
    const onDoc = (e: MouseEvent) => {
      if (wrapRef.current && !wrapRef.current.contains(e.target as Node)) s.setOpen(false);
    };
    document.addEventListener('mousedown', onDoc);
    return () => document.removeEventListener('mousedown', onDoc);
    // s.setOpen は useState の setter で安定。空 deps で 1 度だけ attach する。
        ...
        <div className="absolute left-0 right-0 top-[calc(100%+4px)] z-40 overflow-hidden rounded-[10px] border border-[color:rgba(35,18,0,0.18)] bg-card shadow-[0_12px_32px_rgba(35,18,0,0.13)]">
          {s.error ? (
            <div className="flex items-start gap-2.5 p-3">
              <span className="text-warn"><AlertIcon /></span>
...
function AccessStatus({ value }: { value: AccessValue }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-[10px] border border-accent/20 bg-accent-soft/60 px-3.5 py-3">
      <span className="flex h-[30px] w-[30px] shrink-0 items-center justify-center rounded-lg bg-accent text-white"><CheckIcon /></span>
      <div className="flex-1">
      ...
      <div className="flex flex-col gap-2">
        {AXES.map((axis, i) => (
          <React.Fragment key={axis.key}>
            {i > 0 && (
              <OrConnector
                active={Boolean(nonEmpty[i - 1] && nonEmpty.slice(i).some(Boolean))}
              />
            )}
            <AxisCard
              axis={axis}

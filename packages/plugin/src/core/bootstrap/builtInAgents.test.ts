...
describe('共有定数', () => {
  it('KINTONE_TOOL_NAMES は 13 個 (CRUD 10 + ワークフロー 3)', () => {
    expect(KINTONE_TOOL_NAMES).toHaveLength(13);
    expect(KINTONE_TOOL_NAMES).toContain('kintone-get-records');
    expect(KINTONE_TOOL_NAMES).toContain('kintone-delete-records');
    // プロセス管理 (#22)
    expect(KINTONE_TOOL_NAMES).toContain('kintone-update-record-status');
  ...
  it('ワークフロー系は業務 Agent のみ (customizer-sonnet / designer には出ない)', () => {
    const wf = 'kintone-update-record-status' as const;

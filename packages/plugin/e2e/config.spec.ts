...
test.describe('Cowork Agent — プラグイン設定画面 (3 ステップウィザード)', () => {
  ...
  test('全項目入力 → 保存 → 6 経路の setProxyConfig + REST API deploy まで完走する', async ({
    page,
  }) => {
    test.skip(
    ...
    // 6 経路すべての setProxyConfig が呼ばれるまで待機 (delay があるので最大 60s)
    //   1) /oauth2/token POST
    //   2) Worker /credentials/upsert POST
    //   3) Worker /files/ GET (Issue #29 で追加)
    //   4) Worker /anthropic/ POST (Issue #31 で Anthropic 直接から切替)
    //   5) Worker /anthropic/ GET
    await page.waitForFunction(
      () =>
        ((window as unknown as { __proxyCalls?: unknown[] }).__proxyCalls?.length ?? 0) >= 6,
      { timeout: 60_000 },
    );
    ...
    expect(urls).toContain(`${WORKER_URL}/files/`);
    expect(urls).toContain(`${WORKER_URL}/skills/sync`);
    expect(urls).toContain(`${WORKER_URL}/anthropic/`);

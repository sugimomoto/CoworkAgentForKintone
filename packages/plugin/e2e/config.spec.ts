...
test.describe('Cowork Agent — プラグイン設定画面 (3 ステップウィザード)', () => {
  ...
  test('secret 未入力のとき保存ボタンは無効 (初回 / 再保存ともに)', async ({ page }) => {
    // E2E 環境は credential-bind.setup でほぼ常に isSaved=true になる。
    // workerUrl / clientId は kintone 側に永続化されるが、apiKey / clientSecret は
    // setProxyConfig 側にのみ残るので画面上は未入力。新ロジックは
    // 「secret (apiKey or clientSecret) が 1 つでも入るまで保存不可」なので、
    // 初期表示では保存ボタンは disabled のはず。
    await page.goto(configUrl());
    const root = page.locator('#cowork-agent-config-root');
    await expect(root).toBeAttached({ timeout: 15_000 });
    // 念のため secret 系を空にする
    await root.getByLabel('Anthropic API Key').fill('');
    await root.getByLabel('client_secret').fill('');
  ...
  test('secret 入力で保存ボタンが有効化される (再保存モード)', async ({ page }) => {
    // E2E 環境は isSaved=true 前提。workerUrl + clientId は永続化済み。
    // 新ロジックは「secret (apiKey or clientSecret) が 1 つ入れば再保存可」。
    await page.goto(configUrl());
    const root = page.locator('#cowork-agent-config-root');
    await expect(root).toBeAttached({ timeout: 15_000 });

    // secret 系を空に揃える (再保存時の初期 = 未入力)
    await root.getByLabel('Anthropic API Key').fill('');
    await root.getByLabel('client_secret').fill('');
    ...
    // API Key だけ入れれば有効になる (#29 / #31 のフォロー: API Key 1 つで proxy 再登録可)
    ...
    // client_secret だけでも有効化される
    await root.getByLabel('client_secret').fill('csec');
    await expect(saveBtn).not.toBeDisabled();
  });

  test('全項目入力 → 保存 → 3 経路の setProxyConfig + REST API deploy まで完走する', async ({
    page,
  }) => {
    test.skip(
    ...
    // 3 経路すべての setProxyConfig が呼ばれるまで待機 (delay があるので最大 60s)
    //   1) /oauth2/token POST (kintone 自身のドメイン)
    //   2) Worker root URL POST — 配下の全 POST (/credentials/upsert, /skills/sync, /anthropic/*) を前方一致でカバー
    //   3) Worker root URL GET  — 配下の全 GET (/files/<id>/content, /anthropic/*) を前方一致でカバー
    await page.waitForFunction(
      () =>
        ((window as unknown as { __proxyCalls?: unknown[] }).__proxyCalls?.length ?? 0) >= 3,
      { timeout: 60_000 },
    );
    ...
    const workerRoot = `${WORKER_URL}/`;
    expect(urls).toContain(workerRoot);
    expect(urls.some((u) => u.endsWith('/oauth2/token'))).toBe(true);

    // Worker root POST に 3 つの secret ヘッダがすべて乗っている
    const workerPost = calls.find((c) => c.url === workerRoot && c.method === 'POST');
    expect(workerPost?.headers['X-Anthropic-Api-Key']).toBe(REAL_API_KEY);
    expect(workerPost?.headers['X-Kintone-OAuth-Client-Id']).toBe(REAL_OAUTH_CLIENT_ID);
    expect(workerPost?.headers['X-Kintone-OAuth-Client-Secret']).toBe(REAL_OAUTH_CLIENT_SECRET);

    // Worker root GET にも X-Anthropic-Api-Key
    const workerGet = calls.find((c) => c.url === workerRoot && c.method === 'GET');
    expect(workerGet?.headers['X-Anthropic-Api-Key']).toBe(REAL_API_KEY);

...
test.describe('Cowork Agent — プラグイン設定画面 (3 ステップウィザード)', () => {
  ...
  test('全項目入力 → 保存 → 5 経路の setProxyConfig + REST API deploy まで完走する', async ({
    page,
  }) => {
    test.skip(
    ...
    // 5 経路すべての setProxyConfig が呼ばれるまで待機 (delay があるので最大 60s)
    //   1) /oauth2/token POST
    ...
    await page.waitForFunction(
      () =>
        ((window as unknown as { __proxyCalls?: unknown[] }).__proxyCalls?.length ?? 0) >= 5,
      { timeout: 60_000 },
    );

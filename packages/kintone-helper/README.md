...
alpha 段階は **GitHub Release の wheel URL** から直接取得します (リポジトリ Public のため認証不要):
...
Managed Agents Environment の `packages.pip` にもこの URL をそのまま指定できます。

TestPyPI からも取得可能 (ステージング検証用):
```bash
pip install --extra-index-url https://test.pypi.org/simple/ cowork-agent-kintone==0.1.0a3
```

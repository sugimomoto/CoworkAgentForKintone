...
| 🔥 **V2-新規 (Customizer wedge 実用化)** | V1 のモック状態を本実装に置換。<br>① FileTree を kintone `/k/v1/preview/app/customize.json` から動的構築。desktop.js / mobile.js / desktop.css / 各 URL+FILE リンクを表示・編集可能に。<br>② Customizer Agent の生成出力を **複数ファイル (JS / CSS / mobile.js)** に拡張。<br>③ **apply の実 deploy 実装** ([`kintoneCustomizeApi.ts:buildCustomizeUpdate`](../../packages/plugin/src/chat/workflow/kintoneCustomizeApi.ts) を現状の no-op (= 既存 customize をそのまま PUT) から、artifact JS を FILE upload + PUT customize.json + POST deploy する実装に置換)。<br>④ **preview sandbox の実装** (現状 default no-op、iframe sandbox で artifact JS を実行する実装に置換)。<br>※ V1 は desktop.js 1 本のモック・適用ボタン押下しても本番 customize は変わらない状態 |
...
| **V2-新規 (Customizer wedge 実用化)** | V1 のモック (FileTree hardcoded / apply の deploy が no-op / preview sandbox が no-op) を本実装に置換。FileTree 動的化 + multi-file (JS/CSS/mobile.js) 生成・編集 + apply の実 deploy + preview sandbox 実装。Customizer wedge を実用品にする最重要タスク | V2 |

...
- V2-新規 (Customizer wedge 実用化) V1 のモックを本実装に置換する最重要タスク。
  - ① FileTree 動的化: `/k/v1/preview/app/customize.json` から取得して desktop.js / mobile.js / desktop.css / 各 URL+FILE リンクを表示・編集可能に (現状 hardcoded、クリック no-op)
  - ② Customizer Agent の生成出力を multi-file (JS / CSS / mobile.js) 対応に拡張 (現状 desktop.js 1 本)
  - ③ apply の実 deploy 実装: `kintoneCustomizeApi.ts:buildCustomizeUpdate` を no-op (= 既存 customize をそのまま PUT) から、artifact JS の FILE upload + PUT customize.json + POST deploy する実装に置換 (現状 [適用] ボタン押下でも本番 customize は変わらない)
  - ④ preview sandbox の実装: 現状 default が no-op、iframe sandbox で artifact JS を実行する実装に置換

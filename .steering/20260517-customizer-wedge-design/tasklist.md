...
|---|---|---|---|---|
| **Phase 1** | 基盤型・データ層 | #39 / #40 | 6 | ⬜ 未着手 |
| **Phase 2** | UI 主要 (Header / Settings View) | #39 / #40 | 3 | ⬜ |
| **Phase 3** | Skills 移管 + Plugin Config 縮小 | #40 / #41 | 3 | ⬜ |
| **Phase 4** | Customizer Wedge ループ | #20 | 4 | ⬜ |
...
### [ ] P1.2 — `AgentRecord` / `AgentGlyph` / `AgentColor` 型定義
...
### [ ] P1.3 — `builtInAgents.ts` spec テーブル + system prompt 分割
...
### [ ] P1.4 — `resolveBuiltInAgents` を resolveAgent.ts に追加
...
### [ ] P2.1 — Header (案 C) + `ModelBadge` + `AgentPicker` + `MemoryToggle`
...
### [ ] P2.2 — `SettingsView` + `SettingsNav` shell (空 detail)
...
### [ ] P2.3 — `AgentsListPane` (V1 機能: 公開トグル + AgentIcon)
...
### [ ] P3.2 — `SkillAddModal` (ファイル / 直接入力タブ)
...
### [ ] P3.3 — `ConfigScreen.tsx` から Skills セクションを剥がす (#41)
...
### [ ] P4.1 — `useApplyWorkflow` state machine
...
### [ ] P4.4 — preview / apply / rollback の kintone API 連携

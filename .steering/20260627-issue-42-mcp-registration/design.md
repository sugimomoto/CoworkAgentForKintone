...
- oauth → `mcp_oauth`（access/refresh）。`upsertKintoneCredential` を汎用化した関数を使う。
...
## サーフェス別の実装
### A. Plugin Config（admin・テナント定義） — 詳細は後述「handoff 反映 / Surface A 全面再設計」が正
- `desktop/config/` の ConfigScreen に MCP カタログ ゾーンを統合: 一覧 + 追加/編集/削除フォーム。
  - 入力: name, url, authType。OAuth 時は authorization/token endpoint, clientId, scope, client_secret。
  - 保存: `mcpServers`(JSON) を setConfig。
  - OAuth(confidential) の client_secret は per-server の token endpoint + `/credentials/upsert/{serverId}` への
    proxy 設定を setProxyConfig 登録（kintone と同方式）。allowed_hosts 同期は不要。
...
- `desktop/settings/` の `MCPPanePlaceholder` を実装に置換（admin 専用 2-pane 内、ただし**接続操作は本人**）。
  ...
  - 「接続」: bearer=トークン入力モーダル → proxy 経由で tools/list 検証 → per-user Vault に static_bearer upsert。
    oauth=認可フロー（confidential basic: token 交換時に client_secret を proxy 注入）。
...
### C. Agent 詳細編集（既存 AgentDetailModal に差し込む・#40 依存は解消済み）
...
## Worker への追加
- none/bearer/OAuth(public, secret 不要): 既存 `/credentials/upsert`（static_bearer / mcp_oauth 対応済）で足りる。
- **OAuth(confidential basic)=主モデル → 本スコープで実装**: `/credentials/upsert/{serverId}` を追加（URL の serverId
  限定で proxy 注入された client_secret を Anthropic Vault の `auth.refresh.token_endpoint_auth.client_secret` に積む）。
...
- resources: validateMcpOAuth / listVaultCredentials / archive の単体（既存パターン）。
...
## マイルストーン
→ 下記「handoff 反映」の **更新後マイルストーン** を正とする（旧版は handoff 反映前のため削除）。
...
| `McpAttachSection.tsx` | 既存 `AgentDetailModal` に差込（C / NotifySection と同型・#40 依存解消済み） |
...
- **M3 接続(OAuth)**: 汎用 connectMcpOAuth + mcp_oauth upsert（confidential basic 主・client_secret は Plugin Config/proxy 注入 / public は同フロー吸収 / post 対象外）+ Worker `/credentials/upsert/{serverId}`。
- **M4 attach**: McpAttachSection(C) を既存 AgentDetailModal に差込 + agent mcp_servers/tools 更新 + session vault_ids 拡張（#40 依存は解消済み）。

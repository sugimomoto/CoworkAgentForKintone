...
1. **OAuth 汎用化の経路**（今回スコープ）:
   - **token 交換はプラグインが実施**（kintone 同様 kintone.proxy 経由で token_endpoint を叩く）。Vault は
     交換しない（API は access_token 必須）。以降の refresh のみ Vault が自動（#124 実証）。
   - **PKCE public client（`token_endpoint_auth: none`）を第一級**: client_secret 不要 → secret 注入も不要で最小。
   - **confidential（basic/post）**: client_secret を2経路で proxy 注入しブラウザ非経由にする —
     (a) token 交換時＝token_endpoint への setProxyConfig、(b) Vault refresh 用＝**per-server upsert URL**
     `/credentials/upsert/{serverId}` 限定の proxy header で Worker に渡し
     `auth.refresh.token_endpoint_auth.client_secret` に積む。

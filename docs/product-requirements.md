...
| F-20 | **通知拡張 (Slack / Teams Webhook)** (Agent ごとに 1 本の Incoming Webhook を admin が登録し、`send_notification` ツールで処理結果を Slack / Microsoft Teams に送信。Webhook URL は Anthropic Vault に `static_bearer` で秘匿格納し JS から読めない・ログに出さない。platform は URL ホストで自動判定。チャット・定期実行の両方で利用可) | [#13](https://github.com/sugimomoto/CoworkAgentForKintone/issues/13) |

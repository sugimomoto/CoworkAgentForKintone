...
function VariantRich({ accent = '#6366f1', dark = false, density = 'comfortable', artifactsEnabled = false, initialArtifactId = null, attachments = null, sentAttachments = null }) {
  ...
  const [attachedFiles, setAttachedFiles] = React.useState(() => attachments || []);
  // attachments prop が外から変わったら追従 (Tweaks で切替するため)
  React.useEffect(() => { setAttachedFiles(attachments || []); }, [attachments]);
  const removeAttachment = (id) => setAttachedFiles((prev) => prev.filter((f) => f.localId !== id));
    ...
    <div className="panel" style={{
      ...
      <div ref={scrollRef} className="chat-scroll" style={{
        ...
        {messages.map((m, i) => {
          ...
          return <RichMessage key={i} m={m} c={c} accent={accent} onApprove={() => setApproved(true)} approved={approved} sentAttachments={sentAttachments} />;
        })}
      </div>
      <RichComposer c={c} accent={accent} attachedFiles={attachedFiles} onRemoveAttachment={removeAttachment} />
    </div>
  );
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, lineHeight: 1.25 }}>
        <div style={{ fontSize: 13.5, fontWeight: 600, color: c.text, display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
          <span style={{ whiteSpace: 'nowrap' }}>Cowork Agent for kintone</span>
...
function RichMessage({ m, c, accent, onApprove, approved, sentAttachments }) {
  if (m.kind === 'greeting') return <RichGreeting c={c} accent={accent} />;
  if (m.kind === 'user') {
    // first user message に sentAttachments を表示するサンプル — 実装では m.attachments を読む
    const atts = m.attachments || sentAttachments;
    return (
      <div className="msg-in" style={{ alignSelf: 'flex-end', maxWidth: '85%', display: 'flex', flexDirection: 'column' }}>
        <UserMessageAttachments attachments={atts} c={c} accent={accent} />
        <div style={{
...
function RichComposer({ c, accent, attachedFiles, onRemoveAttachment }) {
  const hasFiles = attachedFiles && attachedFiles.length > 0;
  const hasReading = hasFiles && attachedFiles.some((f) => f.status === 'reading');
  const hasAnyReady = hasFiles && attachedFiles.some((f) => f.status === 'ready');
  // 送信ボタン disabled 条件: reading 中があれば不可。ready ファイルがあるか/テキストがある前提
  const sendDisabled = hasReading;
  return (
    <div style={{
      ...
      <div style={{
        ...
        {hasFiles && (
          <AttachmentChipRow
            files={attachedFiles} c={c} accent={accent}
            onRemove={onRemoveAttachment}
          />
        )}
        <div style={{
          display: 'flex', alignItems: 'flex-end', gap: 8,
          padding: hasFiles ? '6px 8px 8px 14px' : '8px 8px 8px 14px',
        }}>
          <input
            placeholder={hasAnyReady ? '添付について聞く / 指示を入力...' : 'このアプリについて聞く / レコードを操作...'}
            style={{
              flex: 1, border: 'none', outline: 'none', background: 'transparent',
              color: c.text, fontSize: 13, fontFamily: 'inherit', padding: '5px 0',
            }}
          />
          <button style={richIconBtn(c)} title="ファイルを添付 (PDF / 画像 / CSV など)">
          ...
          <button
            ...
            style={{
              ...
              background: sendDisabled ? c.border : accent,
              color: sendDisabled ? c.muted : (c.onAccent || '#fff'),
              cursor: sendDisabled ? 'not-allowed' : 'pointer',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              opacity: sendDisabled ? 0.7 : 1,
              transition: 'background .2s',
            }}
            title={sendDisabled ? '読込中のファイルがあります' : '送信'}
          >
